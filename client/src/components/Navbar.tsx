  ...
  return (
    ...
    >
      <div className="container flex items-center justify-between h-16 gap-2">
        ...
        {/* Desktop Nav */}
        <nav className="hidden md:flex items-center gap-0.5 flex-1 justify-start ml-4">
          {navLinks.map((link) => (
            <Link key={link.href} href={link.href}>
              <span
                className={`relative px-3 py-2 rounded-lg text-xs font-bold tracking-wide whitespace-nowrap leading-none transition-all cursor-pointer uppercase ${
                  isActive(link.href)
                    ? "text-accent"
