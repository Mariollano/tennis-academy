...
// Tracks who has opted in to receive newsletters (email list)
export const newsletterSubscribers = mysqlTable("newsletter_subscribers", {
  id: int("id").autoincrement().primaryKey(),
  email: varchar("email", { length: 320 }).notNull().unique(),
  name: varchar("name", { length: 200 }),
  userId: int("userId"),           // linked user account if they have one
  isActive: boolean("isActive").default(true).notNull(),
  subscribedAt: timestamp("subscribedAt").defaultNow().notNull(),
  unsubscribedAt: timestamp("unsubscribedAt"),
  source: varchar("source", { length: 100 }), // e.g. "booking", "manual", "website"
});
export type NewsletterSubscriber = typeof newsletterSubscribers.$inferSelect;

// ─── Newsletters ──────────────────────────────────────────────────────────────
// Stores each newsletter edition (draft or sent)
export const newsletters = mysqlTable("newsletters", {
  id: int("id").autoincrement().primaryKey(),
  subject: varchar("subject", { length: 500 }).notNull(),
  // Rich content fields
  headline: varchar("headline", { length: 500 }),
  bodyHtml: text("bodyHtml").notNull(),          // full HTML email body
  tennisTip: text("tennisTip"),
  mentalTip: text("mentalTip"),
  winnerSpotlight: text("winnerSpotlight"),       // free-text winner shoutout
  programScheduleHtml: text("programScheduleHtml"), // rendered schedule/pricing table
